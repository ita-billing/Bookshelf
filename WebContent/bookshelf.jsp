...
<body class="background_color">
    ...
    <table border ="1" align="center" class="borderline">
	  ...
	  <tr>
		...
		<td class="table_body center_display"><%=rs.getString(8)%></td>
		<td class="table_body center_display"><%=rs.getString(9)%></td>
		<td class="table_hidden"><%=rs.getString(10)%></td>
			...
			<td>
                <form action="<%=request.getContextPath()%>/change_edit" method="POST" name="edit_form">
                    ...
                    <input type="hidden" name="PUBLISHER" value="<%=rs.getString(4)%>">
                    <input type="hidden" name="PROGRESS" value="<%=rs.getString(5)%>">
                    <input type="hidden" name="PUBLICATIONDATE" value="<%=rs.getString(6)%>">
                    <input type="hidden" name="STARTDATE" value="<%=rs.getString(7)%>">
                    <input type="hidden" name="ENDDATE" value="<%=rs.getString(8)%>">
                    <input type="hidden" name="EVALUATION" value="<%=rs.getString(9)%>">
                    <input type="hidden" name="EVALUATION_VALUE" value="<%=rs.getString(10)%>">
